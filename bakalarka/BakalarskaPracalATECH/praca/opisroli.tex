...
Nakoniec na obrázku \ref{fig:tabulky_pre_role} môžeme vidieť spojenú  relačnú databázu pre potreby serverovej častí rolí.
...
Ďalším rozšírením bude pridanie obmedzení právomocí pre jedného používateľa. Pri modelovaní návrhu rolí v editore ( \ref{Editor manažmentu rolí a organizačnej štruktúry}) chceme pridať ku prechodu možnosť zápornej referencie. To bude znamenať, že jeden používateľ nebude môcť spustiť dva prechody ktoré sa vylučujú. Znamená to teda, že pokiaľ používateľ spustí prvý prechod, nebude schopný vykonať aj druhý prechod, ak obsahuje danú zápornú referenciu. Za zváženie taktiež stojí obmedzenie štruktúry rolí pomocou statického vylúčenia práv. Znamenalo by to tak, že budú existovať určité obmedzenia pri priraďovaní používateľov k rolám. Administrátor by tak nebol schopný priradiť jednému používateľovi dve navzájom sa vylučujúce role.
